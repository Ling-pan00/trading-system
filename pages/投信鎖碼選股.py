...
if st.button(f"🏛️ 啟動 {len(total_pool)} 檔全產業投信鎖碼大數據掃描", type="primary", use_container_width=True):
    ...
    with st.spinner("🚀 正在執行 11 大產業大量 K 線同步與投信鎖碼模型過濾..."):
        try:
            ...
            if df_raw.empty:
                st.error("❌ Yahoo 數據端無回應，請過幾分鐘再重試。")
            else:
                rows = []
                success_count = 0
                ...
                for idx, s_id in enumerate(total_pool):
                    ...
                    try:
                        ...
                        # 【條件一】多頭格局：股價在月線(20MA)與季線(60MA)之上
                        if last['Close'] > last['MA20'] and last['Close'] > last['MA60']:
                            
                            # 【條件二】週轉率安全閥：當日估算週轉率 < 5% (籌碼沉澱，避免當沖炒作)
                            ...
                            if estimated_turnover < 5.0:
                                ...
                                if is_sitc_buying:
                                    ...
                                    sitc_ratio = min(sitc_ratio, 0.25)
                                    
                                    if sitc_ratio >= 0.05:
                                        dist_5ma = (last['Close'] - last['MA5']) / last['MA5']
                                        ...
                                        rows.append({
                                            ...
                                            'sort_key': estimated_turnover # 以週轉率低到高排序，優先展示籌碼最穩個股
                                        })
                    except:
                        continue
                ...
                status_text.success(f"🎉 掃描完成！從 11 大產業成功解析出 {success_count} 檔運行中個股！")
        except Exception as e:
            st.error(f"❌ 發生系統錯誤: {str(e)}")

# ==========================================
# 4. 🎯 畫面呈現：紅綠 K 線與選單 (完全獨立)
# ==========================================
if st.session_state.sitc_report_df is not None:
    ...
    if st.session_state.sitc_report_df.empty:
        ...
        active_list = st.session_state.sitc_report_df['股票代碼'].tolist()
        
    if active_list:
        st.markdown("---")
        st.subheader("📱 投信鎖碼・手機看圖優先區")
        ...
        if user_pick in st.session_state.sitc_stock_cache:
            st.markdown(f"### 📊 **{user_pick}** 籌碼防守 K 線圖 (5/20/60MA)")
            
            df_target = st.session_state.sitc_stock_cache[user_pick].tail(60).copy()
            ...
            ma60_list = [round(v, 2) if not pd.isna(v) else None for v in df_target['MA60'].tolist()]
            
            echarts_options = {
                "backgroundColor": "#121212", 
                "legend": {"data": ["K線", "5MA", "20MA", "60MA"], "textStyle": {"color": "#ffffff"}, "top": "2%"},
                ...
                "series": [
                    ...
                    {"name": "5MA", "type": "line", "data": ma5_list, "smooth": True, "lineStyle": {"opacity": 0.6, "color": "#ffeb3b", "width": 1.2}},
                    {"name": "20MA", "type": "line", "data": ma20_list, "smooth": True, "lineStyle": {"opacity": 0.8, "color": "#e040fb", "width": 1.8}},
                    {"name": "60MA", "type": "line", "data": ma60_list, "smooth": True, "lineStyle": {"opacity": 0.9, "color": "#2196f3", "width": 2.2}}
                ]
            }
            ...
            m2.metric("20MA 月線", f"${round(curr_data['MA20'], 2)}")
            m3.metric("60MA 季線", f"${round(curr_data['MA60'], 2)}")

    st.markdown("---")
    st.subheader("📋 投信鎖碼黑馬對照清單")
    if st.session_state.sitc_report_df.empty:
        st.warning("ℹ️ 今日暫無完全符合「站穩雙線＋投信吃貨＋週轉率<5%」的沉澱鎖碼股。")
    else:
        st.dataframe(st.session_state.sitc_report_df, use_container_width=True)
