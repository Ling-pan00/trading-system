...
st.caption(f"目前時間：{datetime.now(tw_tz).strftime('%Y-%m-%d %H:%M:%S')} | 📱 全產業內建離線版・深夜看盤不卡死")
...
# ==========================================
# 2. 【硬核內建】530檔 上市櫃 11 大產業完整核心池 (免聯網、不卡死)
# ==========================================
def get_industry_stock_pool():
    # 這是依據您的 11 大指定產業（電機/化學/半導體/電腦週邊/光電/通信/電子組件/通路/資服/其他電子/數位雲端）封裝的台股全名單
    full_pool = [
        # --- 上市/上櫃 電機機械 & 化學工業 ---
...
st.write(f"📊 **目前監控守備範圍**：內建 11 大核心指定產業，共計 **{len(total_pool)}** 檔上市櫃個股。")

if st.button(f"🏛️ 啟動 {len(total_pool)} 檔全產業即時盤後大數據掃描", type="primary", use_container_width=True):
    st.session_state.all_stock_cache = {} 
    st.session_state.final_report_df = None
