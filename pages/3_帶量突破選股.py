...
st.write(f"📊 **帶量突破雷達範圍**：共有 {len(total_pool)} 檔標的。")
...
if st.button("⚡ 啟動掃描", type="primary"):
    ...
    with st.spinner("🚀 正在運算量價模型..."):
        ...
        for idx, s_id in enumerate(total_pool):
            ...
            # 策略：突破20日新高 + 今日成交量大於20日均量2倍
            if last_close > high_20 and current_vol > (vol_avg_20 * 2):
                results.append({"代碼": s_id, "收盤價": round(float(last_close), 2), "成交量": int(current_vol)})
                
    if results:
        st.success(f"掃描完成！發現 {len(results)} 檔標的。")
        st.table(pd.DataFrame(results))
    else:
        st.info("今日無符合「強勢突破」條件的標的。")
