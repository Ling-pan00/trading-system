...
def plot_trend_reversal(df, ticker):
    """繪製轉折趨勢圖"""
    order = 5
    close_prices = df['Close'].values
    
    high_idx = argrelextrema(close_prices, np.greater_equal, order=order)[0]
    low_idx = argrelextrema(close_prices, np.less_equal, order=order)[0]
    
    fig = go.Figure()
    fig.add_trace(go.Candlestick(x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], name='股價'))
    
    # 畫趨勢連線
    fig.add_trace(go.Scatter(x=df.index[high_idx], y=df['Close'].iloc[high_idx], mode='lines+markers', name='頭部連線', line=dict(color='red', width=1, dash='dash')))
    fig.add_trace(go.Scatter(x=df.index[low_idx], y=df['Close'].iloc[low_idx], mode='lines+markers', name='底部連線', line=dict(color='green', width=1, dash='dash')))
    
    fig.update_layout(title=f"{ticker} 近三個月趨勢轉折圖", xaxis_rangeslider_visible=False, height=400)
    st.plotly_chart(fig, use_container_width=True)

# 掃描核心
total_pool = get_industry_stock_pool()
st.write(f"📊 監控規模：{len(total_pool)} 檔標的")
...
if st.button("⚡ 啟動掃描", type="primary"):
    ...
    with st.spinner("🚀 正在處理巨量數據..."):
        ...
        for i in range(0, len(total_pool), batch_size):
            ...
            df_raw = yf.download(tickers=batch, start=start_dt, end=end_dt, group_by='ticker', progress=False)
            
            for s_id in batch:
                try:
                    df = df_raw[s_id] if len(batch) > 1 else df_raw
                    ...
                    if last_close > high_20 and df['Volume'].iloc[-1] > (vol_avg_20 * 2):
                        st.subheader(f"✅ 符合條件：{s_id}")
                        plot_trend_reversal(df, s_id)
                except:
                    continue
