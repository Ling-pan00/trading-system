    ...
    st.markdown(f"""
        <div style="background-color: #262730; padding: 15px; border-radius: 10px; color: white;">
            <div style="display: flex; justify-content: space-around; font-size: 20px; font-weight: bold;">
                <span style="color: #FFB74D;">5MA: {df['5MA'].iloc[-1]:.2f} {get_arrow('5MA')}</span>
                <span style="color: #64B5F6;">10MA: {df['10MA'].iloc[-1]:.2f} {get_arrow('10MA')}</span>
            </div>
            <div style="text-align: center; font-size: 20px; font-weight: bold; margin-top: 10px;">
                <span style="color: #BA68C8;">20MA: {df['20MA'].iloc[-1]:.2f} {get_arrow('20MA')}</span>
            </div>
        </div>
    """, unsafe_allow_html=True)
    mc = mpf.make_marketcolors(up='red', down='green', edge='inherit', wick='inherit', volume='in')
    s = mpf.make_mpf_style(marketcolors=mc, gridstyle='--', facecolor='#161616')
    apds = [mpf.make_addplot(df['5MA'], color='#FFB74D', width=1),
            mpf.make_addplot(df['10MA'], color='#64B5F6', width=1),
            mpf.make_addplot(df['20MA'], color='#BA68C8', width=1)]
