...
st.caption(f"目前時間：{datetime.now(tw_tz).strftime('%Y-%m-%d %H:%M:%S')} | 📱 20日新高 + 2倍增量 + 10日波動壓縮")

# 獨立的 Session 記憶池，絕對不與投信策略衝突
...
st.write(f"📊 **帶量突破雷達範圍**：精選與法人同步的 **{len(total_pool)}** 檔上市櫃核心標的。")

if st.button(f"⚡ 啟動 {len(total_pool)} 檔全產業強勢帶量突破掃描", type="primary", use_container_width=True):
    st.session_state.breakout_stock_cache = {} 
    st.session_state.breakout_report_df = None
    
    # 抓取 40 天數據，足夠計算 20 日新高與 20 日均量
            ...
            if df_raw.empty:
                ...
                for idx, s_id in enumerate(total_pool):
                    ...
                    try:
                        ...
                        past_20_days = df_stock.tail(21).iloc[:-1] # 過去20天(不含今天)
                        past_10_days = df_stock.tail(11).iloc[:-1] # 過去10天(不含今天)
                        
                        # 【條件一】股價創 20 日新高 (今天收盤價 > 過去20天的最高收盤價)
                        high_20_day = past_20_days['Close'].max()
                        if last['Close'] > high_20_day:
                            
                            # 【條件二】當日成交量大於 20 日均量的 2 倍 (帶量突破)
                            if last['Volume'] > (last['Vol_MA20'] * 2.0):
                                
                                # 【條件三】過去 10 天橫盤洗盤特徵：振幅(最高/最低差距) < 10%
                                max_p = past_10_days['High'].max()
                                min_p = past_10_days['Low'].min()
                                amplitude_10d = (max_p - min_p) / min_p
                                
                                if amplitude_10d < 0.10:
                                    vol_ratio = last['Volume'] / last['Vol_MA20']
                                    
                                    rows.append({
                                        '股票代碼': s_id,
                                        '今日收盤': round(last['Close'], 2),
                                        '20日最高': round(high_20_day, 2),
                                        '今日成交量': int(last['Volume']),
                                        '20日均量': int(last['Vol_MA20']),
                                        '爆量倍數': f"{round(vol_ratio, 2)}倍",
                                        '10日洗盤振幅': f"{round(amplitude_10d * 100, 2)}%",
                                        'sort_key': vol_ratio # 以爆量倍數由高到低排序，優先展示最強主力表態股
                                    })
                    except:
                        continue
                ...
                status_text.success(f"🎉 突破雷達掃描完成！成功解析 {success_count} 檔有效標的！")
...
if st.session_state.breakout_report_df is not None:
    ...
    if st.session_state.breakout_report_df.empty:
        active_list = list(st.session_state.breakout_stock_cache.keys())
    else:
    ...
    if active_list:
        ...
        if user_pick in st.session_state.breakout_stock_cache:
            st.markdown(f"### 📊 **{user_pick}** 帶量噴發 K 線圖")
    ...
    st.markdown("---")
    st.subheader("📋 橫盤洗盤突破・黑馬對照清單")
    if st.session_state.breakout_report_df.empty:
        st.warning("ℹ️ 今日暫無完全符合「壓縮洗盤完、今日帶量創20日新高」的噴發股。")
    else:
