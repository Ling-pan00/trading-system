...
st.caption(f"掃描股池：565 檔核心個股 | 均線趨勢與波段轉折自動標註")
...
if st.button("🚀 啟動全產業掃描"):
    ...
    with st.spinner("正在執行 565 檔個股深度分析..."):
        for s in total_pool:
            try:
...
if st.session_state.sitc_report_df is not None and not st.session_state.sitc_report_df.empty:
    ...
    st.markdown(f"""
        <div style="background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 6px solid #6c757d; font-family: monospace;">
            <b style="color: #FF9800; margin-right: 20px;">5MA: {get_ma_info('Ma5')}</b>
            <b style="color: #2196F3; margin-right: 20px;">10MA: {get_ma_info('Ma10')}</b>
            <b style="color: #9C27B0;">20MA: {get_ma_info('Ma20')}</b>
        </div>
    ...
    for g, group in df.groupby('Group'):
        if g <= 2: continue
        is_h = group['State'].iloc[0] == 1
        idx = group['High'].idxmax() if is_h else group['Low'].idxmin()
        val = group['High'].max() if is_h else group['Low'].min()
        zigzag_points.append((df.index.get_loc(idx), val, "H" if is_h else "B"))

    # 繪圖
    mc = mpf.make_marketcolors(up='red', down='green', volume='in')
    s = mpf.make_mpf_style(marketcolors=mc, gridstyle='--')
    plots = [mpf.make_addplot(df[['Ma5', 'Ma10', 'Ma20']], width=1)]
    fig, ax = mpf.plot(df, type='candle', style=s, addplot=plots, returnfig=True, figsize=(12, 7), volume=True)
    
    # 畫線與標註
    x_coords = [p[0] for p in zigzag_points]
    y_coords = [p[1] for p in zigzag_points]
    ax[0].plot(x_coords, y_coords, color='black', alpha=0.5, linewidth=1.5, zorder=3)
    for x, y, label in zigzag_points:
        ax[0].text(x, y, label, color='red' if label=="H" else 'green', weight='bold',
                   ha='center', va='bottom' if label=="H" else 'top',
