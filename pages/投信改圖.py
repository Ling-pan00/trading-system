...
st.write(f"📊 **投信鎖碼雷達範圍**：共計 **{len(total_pool)}** 檔個股。")

# ==========================================
# 3. 掃描與運算核心
# ==========================================
if st.button(f"🏛️ 啟動 {len(total_pool)} 檔全產業深度掃描", type="primary", use_container_width=True):
    st.session_state.sitc_stock_cache = {} 
    start_dt = (today_tw - timedelta(days=180)).strftime("%Y-%m-%d")
    end_dt = (today_tw + timedelta(days=1)).strftime("%Y-%m-%d")
    
    with st.spinner("🚀 正在執行大量資料同步與條件過濾..."):
            ...
            try:
                ...
                if last['Close'] > last['MA20'] and last['Close'] > last['MA60']:
                    ...
                    if estimated_turnover < 5.0:
                        ...
                        rows.append({'股票代碼': s_id, '今日收盤': round(float(last['Close']), 2)})
            except: continue
        st.session_state.sitc_report_df = pd.DataFrame(rows)

# ==========================================
# 4. 繪圖呈現 (mplfinance 轉折標註版)
# ==========================================
    ...
    if user_pick in st.session_state.sitc_stock_cache:
        ...
        for g_id, group in df.groupby('STATE_GROUP'):
            ...
            if group['STATE'].iloc[0] == 1:
                ...
                zigzag_points.append((idx, group['HIGH'].max(), "H"))
            else:
                idx = group['LOW'].idxmin()
                zigzag_points.append((idx, group['LOW'].min(), "B"))
        ...
        # 標註轉折點
        if zigzag_points:
            x_vals = [df.index.get_loc(p[0]) for p in zigzag_points]
            y_vals = [p[1] for p in zigzag_points]
            labels = [p[2] for p in zigzag_points]
            
            axlist[0].plot(x_vals, y_vals, color='black', alpha=0.4, linestyle='--')
            for i in range(len(labels)):
                axlist[0].annotate(labels[i], (x_vals[i], y_vals[i]), 
                                 color='red' if labels[i] == 'H' else 'green', 
