...
# 設定 Streamlit 頁面配置
st.set_page_config(page_title="K線橫盤量化選股 Pro", layout="wide")
st.title("📊 K線橫盤整理量化交易系統（單一精選清單 + 800張 + 8%防追高）")
...
def check_strategy(df, price, ma5, ma10, ma20, ma60, s):
    try:
        ...
        # 橫盤定義：過去 20 個交易日的振幅 (最高價 - 最低價) / 最低價 < 10%
        recent_20 = df.tail(20)
        high_20 = recent_20["High"].max()
        low_20 = recent_20["Low"].min()
        amplitude = (high_20 - low_20) / low_20
        
        is_sideways = amplitude < 0.10  # 20日振幅在 10% 以內（緊密箱型整理）

        # 趨勢過濾：確保股價在中長期均線（60MA季線）之上，避免選到空頭死魚股
        above_ma60 = price > ma60
        ma60_up = ma60 > ma60 * 0.99  # 季線持平或向上

        if is_sideways and above_ma60 and ma60_up and s >= 4:
            return True
...
def draw_zigzag_chart(ticker_code, stock_name):
    ...
    st.markdown(f"#### 📈 {stock_name} ({ticker_code}) — K線橫盤整理 5MA 轉折波段圖")
    st.markdown(f"""
        <div style="
...
if st.button("🚀 執行 K線橫盤策略選股"):
    ...
    if not results:
        st.warning("⚠️ 經過成交量（800張）、K線橫盤整理與防追高限制（8%）篩選後，目前沒有符合標準的標的。")
...
if "qualified_stocks" in st.session_state:
    ...
    st.subheader(f"📊 K線橫盤策略精選總名單（共 {len(saved_df)} 檔）")
