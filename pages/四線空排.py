...
def check_strategy(df, price, ma5, ma10, ma20, ma60, s):
    try:
        ...
        # 核心條件：四線空頭排列 (5MA < 10MA < 20MA < 60MA)
...
def trade_levels(price, ma5, ma10):
    stop = ma10             # 空方防守：以 MA10 作為停損防守價（價格若突破 MA10 則停損）
    target = price * 0.85   # 空方波段目標價：期待下跌 15%
...
def draw_zigzag_chart(ticker_code, stock_name):
    ...
    st.markdown(f"#### 📈 {stock_name} ({ticker_code}) — 四線空排 5MA 轉折波段圖")
    st.markdown(f"""
        <div style="
...
if st.button("🚀 執行四線空排策略選股"):
    ...
    status_text.text("🎉 四線空排策略精選選股完成！")

    if not results:
        st.warning("⚠️ 經過成交量（800張）、四線空排與防風險限制（8%）篩選後，目前沒有符合標準的標的。")
...
if "qualified_stocks" in st.session_state:
    ...
    st.subheader(f"📊 四線空排策略精選總名單（共 {len(saved_df)} 檔）")
...
def run_monitor_optimized(pool_df):
    ...
    for _, row in pool_df.iterrows():
        ...
        try:
            ...
            low_5 = df["Low"].iloc[-6:-1].min()
            vol_today = df["Volume"].iloc[-1]
            vol_avg = df["Volume"].rolling(5).mean().iloc[-1]

            black_k = close_now < open_now
            below_ma5 = close_now < ma5
            breakdown = close_now < low_5
            vol_ok = vol_today > vol_avg

            if black_k and below_ma5 and vol_ok and breakdown:
                signal = "🟢 空方強力SELL (量增破底)"
            elif black_k and below_ma5:
                signal = "🟡 空方WATCH (轉弱向下)"
            else:
                signal = "🔴 NO (空方型態受阻/反彈)"
            ...
            live_results.append({
                ...
                "即時MA5": round(ma5, 2),
                "黑K": "✅收黑" if black_k else "❌收紅",
                "跌破MA5": "✅" if below_ma5 else "❌",
                "量能放大": "✅量增" if vol_ok else "❌量縮",
                "跌破近5日低": "✅破底" if breakdown else "❌未破",
                "📢 盤中即時訊號": signal
            })
        except:
            continue
