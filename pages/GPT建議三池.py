...
if "breakout" in st.session_state:
    ...
    if st.button("🔄 更新訊號"):
        for label, key in pools.items():
            st.markdown(f"**{label}**")
            live = [{"代號": row["代號"], "訊號": "🟢 BUY" if yf.download(row["ticker"], period="5d", progress=False)["Open"].iloc[-1] >= yf.download(row["ticker"], period="5d", progress=False)["Close"].iloc[-2] else "🟡 WATCH"} for _, row in st.session_state[key].iterrows()]
    ...
    fig, axlist = mpf.plot(df_k.iloc[-90:], type='candle', returnfig=True, volume=True, figsize=(12, 8),
                           addplot=[mpf.make_addplot(df_k['5MA'].iloc[-90:], color='orange'),
    ...
    ax.plot(x, y, color='gray', linestyle='-', linewidth=1.5, zorder=2)
    
    # 繪製 H/B 標記
    for idx, val, lbl in points:
        if idx in df_k.iloc[-90:].index:
            ax.annotate(lbl, (df_k.index.get_loc(idx), val), xytext=(0, 10 if lbl=='H' else -10),
                        textcoords='offset points', ha='center', color='red' if lbl=='H' else 'green',
                        weight='bold', bbox=dict(fc="white", ec='red' if lbl=='H' else 'green', pad=1), zorder=3)
    st.pyplot(fig)
