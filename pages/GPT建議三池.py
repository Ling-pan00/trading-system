...
if "breakout" in st.session_state:
    ...
    fig, axlist = mpf.plot(df_k.iloc[-90:], type='candle', returnfig=True, volume=True, figsize=(12, 8),
                           addplot=[mpf.make_addplot(df_k['5MA'].iloc[-90:], color='orange', width=1.5)])
    ax = axlist[0]
    
    for idx, val, lbl in points:
        if idx in df_k.iloc[-90:].index:
            ax.annotate(lbl, (df_k.index.get_loc(idx), val), xytext=(0, 15 if lbl=='H' else -15), textcoords='offset points',
