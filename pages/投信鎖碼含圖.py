...
if st.button("開始 V9.2"):
    ...
    for stock, g in df.groupby(stock_col):
        try:
            ...
            result.append({
                "股票": stock, "強度": round(strength, 4), "穩定度": round(stability, 4),
                "近3日買超": int(last3_sum), "近10日買超": int(last10_sum)
            })
        except: continue
    ...
    # 從 session_state 取出結果進行分析
    final_out = st.session_state.get('final_out')
    sel = st.selectbox("分析個股：", final_out["股票"].tolist())
    
    # 轉換股票代號為 yfinance 格式 (假設為台股)
    ticker = f"{sel}.TW"
